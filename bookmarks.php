...
<body>
    ...
    if (isset($_SESSION['signedIn']) && $_SESSION['signedIn'] == true) {
        ...
        if (isset($_GET['studentID'])) {
            ...
            $bookmarksQuery = mysqli_query($conn, "SELECT * FROM bookmarks WHERE bookmarks.studentID = '" . $studentID . "' ;");

            while ($bookmarksArr = mysqli_fetch_array($bookmarksQuery)) {

                if ($bookmarksArr['slateID'] != NULL || $bookmarksArr['slateID'] != '') {

                    $slatesQuery = mysqli_query($conn, "SELECT * FROM slate WHERE slate.slateID = '" . $bookmarksArr['slateID'] . "' ORDER BY createdAt DESC");
