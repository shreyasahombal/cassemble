...
<body>
    ...
    if (isset($_GET['eventID'])) {
        ...
        $studentsQuery = mysqli_query($conn, "SELECT student.studentID, student.imageURL, college.logoURL, student.name as studentName, stream.name as streamName, student.collegeID, college.name as collegeName, city.cityID, city.name as cityName, state.stateID, state.name as stateName, student.streamYear FROM eventsandstudent, student, college, city, state, stream WHERE eventsandstudent.eventID = '".$eventID."' AND student.studentID = eventsandstudent.studentID AND college.collegeID = student.collegeID AND city.cityID = college.cityID AND state.stateID = college.stateID AND stream.streamID = student.streamID ;");
